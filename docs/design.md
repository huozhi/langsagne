...
"Directive" means the small instruction generated by the compiler. The VM does not execute source text directly; it executes directives.
...
c4 handles system calls as built-in VM directives. During startup, it inserts names like `open`, `read`, `printf`, `malloc`, and `exit` into the symbol table. When the parser sees a call to one of those names, it emits the matching syscall directive instead of a normal function jump. The VM then runs the host operation directly and leaves the result in the accumulator.

This project keeps only the shape for now:
...
`print(expr)` is the only current system call. It evaluates one expression, writes the value with JavaScript `console.log`, and leaves that same value in `ax`. Do not add c4's file, memory, or `printf` calls until this language has strings, multiple arguments, and a clearer memory model.

## Runtime Shape

Keep the runtime closer to a small JavaScript interpreter than to c4's full C machine. Avoid modeling C memory, OS syscalls, pointer arithmetic, `malloc`, `printf`, or byte-addressed character storage until there is a clear experiment that needs them.
...
For `1 + 2 * 3`, the generated directives can look like this:
...
The value stack (`vs`) grows when the VM must pause one calculation to start another. It shrinks when a directive combines the remembered value with `ax`.
